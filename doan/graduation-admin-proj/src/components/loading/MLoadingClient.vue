    ...
    <div id="preloader">
        ...
        <div id="deadline">
            <svg preserveAspectRatio="none" id="line" viewBox="0 0 581 158" enable-background="new 0 0 581 158">
                ...
                <g id="death-group" :style="{ 'animation-duration': animationTime + 's' }">
                    <path
                        id="death"
                        fill="#BE002A"
            ...
            <!-- <div class="deadline-days">Deadline <span class="day">7</span> <span class="days">days</span></div> -->
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        deadlineAnimation() {
            const animations = [1.5, 1, 0.7, 0.3, 0.2];
            animations.forEach((duration, index) => {
                setTimeout(() => {
                    this.armAnimationDuration = `${duration}s`;
                }, index * 4000);
            });
        },

        timer(totalTime, deadline) {
            const time = totalTime * 100;
            const dayDuration = time / deadline;
            this.actualDay = deadline;
